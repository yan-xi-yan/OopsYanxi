...
> [!info] 权重共享
> 许多模型（GPT-2、T5 等）让输出投影矩阵 $W$ 和输入 [Embedding 矩阵](../02_Input_Representation/01_Token Embedding.md) $W_E$ 共享权重（Weight Tying），即 $W = W_E$。这不仅减少参数量，还在语义上合理——一个词的"输出表示"和"输入表示"应该是一致的。
...
- [Masked Self Attention](./02_Masked Self Attention.md) — 上一篇：解码器的因果掩码
